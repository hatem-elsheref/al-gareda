    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            <h1>
                الكلمات المفتاحية
                <small>
                    <span class="badge btn-danger btn-sm">{{count($tags)}}</span>
                </small>
                    ...
                    <form method="get" action="{{route('tags.index')}}">
                        ...
                        <div class="col-md-5">
                            ...
                            @if(auth()->user()->hasPermission('create_tags'))
                                <a href="{{route('tags.create')}}" class="brn btn-sm btn-danger"><span class="fa fa-plus"></span> اضافة كلمه مفتاحية جديده </a>
                            @else
                ...
                <div class="col-md-12">
                    <div class="box box-success">
                        ...
                        <div class="box-body">
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @if(auth()->user()->hasPermission('delete_tags'))
                                                    <form method="post" id="remove_user_form_{{$user->id}}" action="{{route('tags.destroy',$user->id)}}">
                                                        {{csrf_field()}}
                                                        {{method_field('DELETE')}}
                                                    </form>
                                                ...
                                                @if(auth()->user()->hasPermission('update_tags'))
                                                    <a href="{{route('tags.edit',$user->id)}}" class="btn btn-sm btn-success"><i class="fa fa-edit"></i> تعديل</a>
                                                @else
                            ...
                            {!! $tags->appends(['search'=>request()->query('search')])->render() !!}
