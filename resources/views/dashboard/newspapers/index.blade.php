    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            <h1>
                الجرائد
                <small>
                    <span class="badge btn-danger btn-sm">{{count($newspaper)}}</span>
                </small>
        ...
        <section class="content">
            <div class="row mb-4">
                <div class="col-md-7  mr-bottom-10">
                    <form method="get" action="{{route('newspapers.index')}}">
                        ...
                        <div class="col-md-5">
                            ...
                            @if(auth()->user()->hasPermission('create_newspaper'))
                                <a href="{{route('newspapers.create')}}" class="brn btn-sm btn-danger"><span class="fa fa-plus"></span> اضافة جريده جديد </a>
                            @else
                ...
                <div class="col-md-12">
                    <div class="box box-success">
                        ...
                        <div class="box-body">
                                ...
                                @else
                                    ...
                                    @foreach($newspaper as $user)
                                        <tr>
                                            ...
                                            <td style="text-align: center"><img src="{{url('storage/newspapers/'.$user->photo)}}" style="width: 60px;height: 60px;"></td>
                                            <td>
                                                @if(auth()->user()->hasPermission('delete_newspaper'))
                                                    <form method="post" id="remove_user_form_{{$user->id}}" action="{{route('newspapers.destroy',$user->id)}}">
                                                        {{csrf_field()}}
                                                        {{method_field('DELETE')}}
                                                    </form>
                                                ...
                                                @if(auth()->user()->hasPermission('update_newspaper'))
                                                    <a href="{{route('newspapers.edit',$user->id)}}" class="btn btn-sm btn-success"><i class="fa fa-edit"></i> تعديل</a>
                                                @else
                            ...
                            {!! $newspaper->appends(['search'=>request()->query('search')])->render() !!}
