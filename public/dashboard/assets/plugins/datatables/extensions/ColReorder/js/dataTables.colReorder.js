...
(function (window, document, undefined) {
    ...
    /**
     * Switch the key value pairing of an users array to be value key (i.e. the old value is now the
    ...
    var factory = function ($, DataTable) {
        ...
        $.fn.dataTableExt.oApi.fnColReorder = function (oSettings, iFrom, iTo) {
            ...
            if (iFrom < 0 || iFrom >= iCols) {
                this.oApi._fnLog(oSettings, 1, "ColReorder 'from' users is out of bounds: " + iFrom);
                return;
            }

            if (iTo < 0 || iTo >= iCols) {
                this.oApi._fnLog(oSettings, 1, "ColReorder 'to' users is out of bounds: " + iTo);
                return;
            }

            /*
             * Calculate the new column array users, so we have a mapping between the old and new
             */
            var aiMapping = [];
            ...
            if (oSettings.aoColumns[iFrom].bVisible) {
                /* Calculate the current visible users and the point to insert the node before. The insert
        ...
        ColReorder.prototype = {
            ...
            "_fnMouseDown": function (e, nTh) {
                ...
                var idx = parseInt($(nTh).attr('data-column-users'), 10);
            ...
            "_fnDestroy": function () {
                ...
                $.each(this.s.dt.aoColumns, function (i, column) {
                    $(column.nTh).removeAttr('data-column-users');
                });
            ...
            /**
             * Add a data attribute to the column headers, so we know the users of
             * the row to be reordered. This allows fast detection of the users, and
             * for this plug-in to work with FixedHeader which clones the nodes.
             *  @private
             */
            "_fnSetColumnIndexes": function () {
                $.each(this.s.dt.aoColumns, function (i, column) {
                    $(column.nTh).attr('data-column-users', i);
                });
            }
        };
